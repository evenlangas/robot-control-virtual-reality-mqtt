...
class JoinSubscriberMqttNode(Node):
    def __init__(self):
        super().__init__("joint_subscriber_mqtt")
        self.node_red_publisher_ = self.create_publisher(String, "/cobot_to_unity_mqtt", 10)
    ...
    def joint_callback(self, msg: JointState):
        ...
        if not np.allclose(current_positions_deg, self.previous_position_deg, atol=0.01):
            if not self.moving:
                ...
                # If already moving, just reset the stable_iteratiros2 run joint_listener joint_subscriberons
                self.stable_iterations = 0
        ...
        self.cmd.data = "{"
        self.cmd.data += '"joint_1": ' + str(current_positions_deg[0]) + "," 
        self.cmd.data += '"joint_2": ' + str(current_positions_deg[1]) + "," 
        self.cmd.data += '"joint_3": ' + str(current_positions_deg[2]) + "," 
        self.cmd.data += '"joint_4": ' + str(current_positions_deg[3]) + "," 
        self.cmd.data += '"joint_5": ' + str(current_positions_deg[4]) + "," 
        self.cmd.data += '"joint_6": ' + str(current_positions_deg[5]) 
        self.cmd.data += "}" 
